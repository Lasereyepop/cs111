...
class TestLab1Extra(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if result.returncode != 0:
            ...
            return
        result = subprocess.run(
            ["cc", "pipe.o", "-o", "pipe"], capture_output=True, text=True
        )
    ...
    def test_bogus_first(self):
        ...
        pipe_result = subprocess.run(
            ("./pipe", "bogus", "ls"), stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        self.assertEqual(
            pipe_result.returncode,
            22,
            msg=f"Bogus first argument should exit with EINVAL (22), got {pipe_result.returncode}.",
        )
        self.assertNotEqual(
            pipe_result.stderr, b"", msg="Error should be reported to standard error."
        )

    def test_bogus_middle(self):
        self.assertTrue(self.make, msg="build failed")
        pipe_result = subprocess.run(
            ("./pipe", "cat", "bogus", "wc"),
            input=b"hello\nworld\n",
        ...
        self.assertEqual(
            pipe_result.returncode,
            22,
            msg=f"Bogus middle argument should exit with EINVAL (22), got {pipe_result.returncode}.",
        )
        self.assertNotEqual(
            pipe_result.stderr, b"", msg="Error should be reported to standard error."
        )
    ...
    def test_single_cat_passthrough(self):
        self.assertTrue(self.make, msg="build failed")
        pipe_result = subprocess.run(
            ("./pipe", "cat"), input=b"hello\nworld\n", capture_output=True, timeout=2
        )
        self.assertEqual(pipe_result.returncode, 0)
        self.assertEqual(pipe_result.stdout, b"hello\nworld\n")
        self.assertEqual(pipe_result.stderr, b"")

    def test_pipeline_with_input(self):
        self.assertTrue(self.make, msg="build failed")
        cl_result = subprocess.run(
            "cat | cat | wc", input=b"abc def\nxyz\n", capture_output=True, shell=True
        )
        pipe_result = subprocess.run(
            ("./pipe", "cat", "cat", "wc"),
            input=b"abc def\nxyz\n",
        ...
        self.assertEqual(
            cl_result.stdout,
            pipe_result.stdout,
            msg=f"Pipeline with input: expected {cl_result.stdout} but got {pipe_result.stdout}.",
        )

    def test_empty_input_no_hang(self):
        self.assertTrue(self.make, msg="build failed")
        cl_result = subprocess.run(
            "cat | wc", input=b"", capture_output=True, shell=True
        )
        pipe_result = subprocess.run(
            ("./pipe", "cat", "wc"), input=b"", capture_output=True, timeout=2
        )
        self.assertEqual(pipe_result.returncode, 0)
        self.assertEqual(
            cl_result.stdout,
            pipe_result.stdout,
            msg=f"Empty input: expected {cl_result.stdout} but got {pipe_result.stdout}.",
        )

    def test_more_than_eight_commands(self):
        self.assertTrue(self.make, msg="build failed")
        cl_result = subprocess.run(
            "cat | cat | cat | cat | cat | cat | cat | cat | wc",
            input=b"one\ntwo\n",
            capture_output=True,
            shell=True,
        )
        pipe_result = subprocess.run(
            ("./pipe", "cat", "cat", "cat", "cat", "cat", "cat", "cat", "cat", "wc"),
            input=b"one\ntwo\n",
            capture_output=True,
            timeout=2,
        )
        ...
        self.assertEqual(
            cl_result.stdout,
            pipe_result.stdout,
            msg=f"More than eight commands: expected {cl_result.stdout} but got {pipe_result.stdout}.",
        )
    ...
    def test_success_stderr_empty(self):
        ...
        self.assertEqual(
            pipe_result.stderr,
            b"",
            msg=f"Expected empty stderr on success, got {pipe_result.stderr}.",
        )
